package school.programmers.l2.miner;
...
//-------- 1. 범위 0 ~ 광물배열 길이 또는 0 ~ 곡갱이 갯수*5중 작은값으로 설정하고 i%5==0일 때마다 아래 코드 실행 --------//
//-------- 2. 5칸씩(끝날때는 남은 미네랄 길이) 끊어서 5칸(또는 그 이하)짜리 배열생성 후 피로도 정보 입력 --------//
//-------- 3. 리스트내 배열을 입력정보 합 기준으로 정렬하는데 만약 같으면 광물 수 기준으로 정렬 --------//
//-------- 4. 정렬 후 높은 값 부터 순서대로 다이아->철->돌 사용시 피로도 합 계산 --------//
//-------- 5. 위에서 피로도 계산을 5^2,5^1,5^0 로 정리했기 때문에 다이아곡갱이->나누기 25, 철->나누기 5, 돌->나누기 1 (최소값 1로 제한) 해서 피로도 계산이 가능 --------//

// 차이 : 2번에서 피로도정보를 곡갱이별로 입력, 1번이후 사용할 곡갱이 수 파악, 5번에서 사용할 곡갱이 수 대로 순서대로 넣어서 계산
